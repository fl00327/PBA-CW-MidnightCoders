...
#Map all NA values to 0.
df$cigarettes_per_day <-ifelse(is.na(df$cigarettes_per_day), 0, df$cigarettes_per_day)
...
earthGrid <-  expand.grid(nprune = seq(0, 10, by = 2), 
                          degree = seq(0, 10, by = 2))
...
fit0 <- train(weight_pounds ~ .,   #Predict weight_pounds using all other fields in dataframe
              ...
              tuneGrid = earthGrid, #Hyperparameter Tuning, changing nprune and degree
...
#Train a new model with the best parameters hard-coded in, removing the need to try every possible combination again.
fit2 <- train(weight_pounds ~ is_male+plurality+mother_age+gestation_weeks+cigarettes_per_day,   #Predict weight_pounds using all other fields in dataframe
              data=dfTrain,        #Training dataset - 70% of original dataset
              method='earth',      #Multivariate adaptive regression splines (MARS)
...
GetModelAccuracy(fit1,dfTest)
GetModelAccuracy(fit2,dfTest)
